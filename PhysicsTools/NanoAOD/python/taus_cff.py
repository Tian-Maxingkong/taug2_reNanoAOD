...
_tauVarsBase = cms.PSet(P4Vars,
       ...
       # added: per-user request, include second/third signal charged candidates' dz and first three signal charged candidate four-vectors
       dz2 = Var("?signalChargedHadrCands().size()>1?signalChargedHadrCands()[1].dz():-99",float, doc="d_{z} of second track with respect to PV, in cm (with sign)",precision=14, lazyEval=True),
       dz3 = Var("?signalChargedHadrCands().size()>2?signalChargedHadrCands()[2].dz():-99",float, doc="d_{z} of third track with respect to PV, in cm (with sign)",precision=14, lazyEval=True),
