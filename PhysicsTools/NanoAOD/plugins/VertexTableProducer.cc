...
void VertexTableProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  auto pfcIn = iEvent.getHandle(pc_);
  ...
  for (size_t i = 0; i < (*pvsIn).size() && i < 5; i++){
     ...
     int ntr0p5=0;
     int ntr0p9=0;
     for (auto iter_pf = pfcIn->begin(); iter_pf != pfcIn->end(); iter_pf++) {
  ...
  otherPVsTable->addColumn<float>(
      "z", pvsz, "Z position of other primary vertices", 8);
  otherPVsTable->addColumn<int>(
      "ntracks0p9", pvsntracks0p9, "Number of tracks of primary vertices pt>0.9");
  otherPVsTable->addColumn<int>(
      "ntracks0p5", pvsntracks0p5, "Number of tracks of primary vertices pt>0.5");
...
void VertexTableProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("packedPFCandidates1")->setComment("packedPFCandidates input collections");
  ...
  desc.add<bool>("storeCharge", true)->setComment("whether to store the sum charge of SV tracks");
